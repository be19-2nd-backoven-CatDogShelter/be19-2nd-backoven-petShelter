...
DROP TABLE IF EXISTS volunteerAssociationFiles;
...
CREATE TABLE user (
    ...
    cumulative_volunteer_time INT NULL,      -- 누적봉사시간(nullalbe)
    month_volunteer_time INT NULL,           -- 월봉사시간(nullalbe)
    volunteer_count INT NULL,                -- 봉사횟수
    deactivation_times INT NULL,             -- 블라인드 횟수(nullalbe)
    user_status ENUM('GENERAL','BLACK','CANCEL') NOT NULL default 'GENERAL', -- 회원상태
    activation_date VARCHAR(20) NULL,        -- 정지 해제일
    rating_id INT NOT NULL default 0,        -- 회원등급 FK(0)
...
CREATE TABLE volunteerAssociation (
    ...
    PRIMARY KEY (id),
    CONSTRAINT fk_va_head FOREIGN KEY (head_id) REFERENCES shelter_head(head_id),
    CONSTRAINT fk_va_sigungu FOREIGN KEY (sigungu_id) REFERENCES sigungu(sigungu_id)
);

-- 봉사모임 업로드 파일 --
CREATE TABLE volunteerAssociationFiles (
    id INT NOT NULL AUTO_INCREMENT,
    ...
    PRIMARY KEY (id),
    CONSTRAINT fk_vaf_association FOREIGN KEY (association_id) REFERENCES volunteerAssociation(id)
);
