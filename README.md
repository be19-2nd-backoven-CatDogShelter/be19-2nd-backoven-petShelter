...
| 이승건  | 이번 프로젝트는 유기동물 커뮤니티 백엔드 개발을 목표로 진행하였다. 아키텍처는 CQRS 패턴을 적용하여 Query 파트는 MyBatis로, Command 파트는 JPA로 구현하였다. REST API를 지향하며 메소드의 기능에 맞게 매핑을 진행했으나, URL 명칭까지 완전히 RESTful하게 적용하지 못한 점은 아쉬움으로 남는다.
프로젝트 초기에 DDD 설계를 진행한 덕분에 개발 과정에서 흐름을 놓치지 않고 팀원들과 원활하게 소통할 수 있었다. 도메인 언어를 통일함으로써 개발 방향성을 공유하는 것이 얼마나 중요한지 체감할 수 있었다.
DDD 기반으로 여러 종류의 게시판이 도출되었고, 팀원 모두가 DB-Java CRUD 경험을 쌓을 수 있도록 각자 하나 이상의 게시판을 맡았다. 나는 목격 정보 게시판, 등급·봉사왕 기능, 쪽지 기능을 담당하였다. 기능 분배 당시에는 게시판 외의 기능을 맡고 싶어 자발적으로 선택했지만, 상대적으로 가벼운 기능이어서 다른 게시판을 맡아볼 걸 하는 아쉬움도 있었다.
협업 과정에서는 규칙 부재로 인해 클래스명이나 파일명 충돌이 빈번하게 발생했다. 이를 해결하기 위해 도메인명을 접두사로 붙이는 방식을 사용했지만, 애초에 규칙을 정하지 않은 것이 문제의 원인이었다. 이 경험을 통해 DDD와 같은 설계 방식이 팀의 언어를 통일하고 방향성을 잡는 데 중요하다는 점, 그리고 협업을 위한 명확한 규칙 설정이 필수적이라는 점을 배울 수 있었다. |
