...
|--------|-------|
| 곽동근  | (회고 내용) |
| 변민철  | DB 프로젝트 이후 본격적인 애플리케이션 개발에 참여하며, 이전에 막연하게만 알던 객체지향 개념이 실제 경험으로 다가왔다. 처음에는 의존성 같은 용어가 추상적으로만 느껴졌지만, 스프링을 학습하면서 “의존성을 주입받는다”, “다른 클래스를 의존한다”와 같은 상황을 직접 겪으며 점차 그 의미를 이해할 수 있었다. 이번 프로젝트를 통해 가장 크게 깨달은 것은 체계 없는 개발은 곧 큰 문제로 이어진다는 점이었다. 실제로 클래스 이름을 임의로 정하다가 스프링이 생성하는 Bean 이름이 충돌하는 문제가 발생했고, 여러 패키지와 코드를 수정해야 했다. 이를 통해 작은 규칙 하나가 팀 전체의 개발 흐름에 큰 영향을 미친다는 사실을 배웠다. 기술적으로는 MyBatis를 SQL의 연장선으로 이해할 수 있었고, Mapper.xml과 동적 쿼리에 익숙해졌다. 반면 JPA는 Entity-테이블 관계와 영속성 컨텍스트 등 더 깊은 학습이 필요함을 느꼈다. 결국 이번 프로젝트에서 얻은 가장 큰 교훈은, 개발은 기술 이전에 체계와 원칙 속에서 이루어져야 한다는 것이다. 올바른 기반 위에서 협업을 이어갈 때 기술적 성장은 자연스럽게 따라온다는 확신을 얻었다. |
| 이승건  | (회고 내용) |
