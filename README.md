...
| 조원명 | 회고 |
|--------|-------|
| 곽동근  | 이번 프로젝트는 Spring Boot 기반 MVC를 기본으로 하고, DDD설계를 채용하고 CQRS패턴을 적용하여 백엔드 프로젝트를 진행하였는데 프로젝트 주제에 대한 도메인이 여러개 나눠져있어서 열심히 배웠던 내용들을 각자 최대한 활용해서 팀원들이 각자 맡은 모듈에 잘 녹일 수 있었던 것 같다. 그리고 다들 열정적이고 능동적으로 잘 움직이는 사람들이 모여있어서 프로젝트 진행하는동안 일정에 대한 차질도 없었고 서로 모르는 부분들은 보완해가면서 성장하고 프로젝트가 잘 마무리 될 수 있었다. 그리고 프로젝트 거의 끝나갈무렵 security, MSA 아키텍처를 학습 한 후에 설계해서 프로젝트를 확장했는데 어렵고 이해가 안가는 부분들도 있어서 이 부분은 따로 학습을 해보고, 이 부분 외로 시간을 내서 백엔드 공부를 좀 더 해봐야될것같다.
그리고 이번 프로젝트 일정에 대한 차질이 없긴했지만 아무래도 빠듯한 일정이였다보니 신경을 못 쓴 부분들에 대해서 트러블슈팅이 발생했었다.
트러블슈팅을 겪은 첫번째로는, 처음 패키지 구조 설계할때 구조에 대한 설계뿐만 아니라  각 도메인별로 클래스까지 생성해둔 상태로 패키지 구조를 뿌리고 각자 개발했었는데 각 도메인별로 스프링 컨테이너에 Bean 이름이 등록될때 이름이 겹치면 안되는데 Entity, Controller, repository, Service 클래스 이름들을 전부 같게 해뒀었어서 나중에 모듈을 합쳤을때 쉽지않았었다. 그래도 초반에 회의를 진행하고 각자 맡은 도메인 클래스들 이름을 빠르게 수정해서 바로잡았고
다음으로 겪은문제는 첫번째때 겪은 문제를 나름 빨리 잡았다 생각했었는데 첫번째에 겪은 문제+모두 중간 중간 개발하고 PR날렸어야됐는데 이런 과정들이 많이 없었어서 병합 충돌이 크게 일어났었고 해당 부분들을 수정하는데 시간이 좀 오래 걸리긴했지만 좋은 경험과 큰 깨달음을 얻은 프로젝트 였던것같다. |
